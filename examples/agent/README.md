...
The examples that used to be here called a hallpass server through `hallpass_client` (the old
Python client, now replaced by `hallpass`: `Hallpass.remote()` is the same client) and wrapped
each tool with `guarded`, which is `hallpass.guarded`. The adapters' tests in
