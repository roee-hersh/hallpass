...
def _compose(data: bytes | str) -> _Node | None:
    ...
    try:
        ...
        raise _lookahead_error(data, e.problem_mark) or e from None
    except _YAMLFailure as e:
        raise _lookahead_error(data, e.mark) or e from None
    finally:
        _dispose(c.p)
    if doc is not None:
        err = _lookahead_error(data, c.end.start_mark)
        if err is not None:
            raise err
    return doc
...
def _lookahead_error(data: bytes | str, mark: Any) -> Exception | None:
    """The scanner error yaml.v3 meets first, if any. Its scanner keeps two
    tokens more in hand than libyaml's (for comments), so a scanner error in
    the two tokens after the one the parser stopped at is what it reports,
    in place of libyaml's parser error or success."""
    if mark is None:
        return None
    p = _event_parser(data)
    try:
        extra = -1
        while extra < 2:
            tok = p.get_token()
            ...
            if extra >= 0:
                extra += 1
            elif tok.start_mark.index >= mark.index:
                extra = 0
    except (yaml.scanner.ScannerError, yaml.reader.ReaderError) as e:
        return e
    finally:
