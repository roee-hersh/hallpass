...
def fw_langchain_vault() -> Iterator[str]:
...
def test_real_vault(fw_langchain_vault: str) -> None:
    hp = vault_hallpass(fw_langchain_vault)
    ...
    m = mw(hp, {"write_secret": ("vault", "secret.write", "kv:secret/{path}")})
    results = [
        run_agent([("write_secret", {"path": path, "value": "x", "user_id": ADMIN})], Ctx(user), [m], [write_secret], use_async=use_async)[0]
        for user, path, use_async in ((ADMIN, "app/db", False), (DANA, "app/db", True), (ADMIN, "other/db", False))
    ]
    assert [r.status for r in results] == ["success", "error", "error"]
    assert str(results[1].content).startswith(REFUSED + "dana@example.com may not secret.write on kv:secret/app/db in vault: deny (denied: ")
    assert str(results[2].content).startswith(REFUSED + "admin@example.com may not secret.write on kv:secret/other/db in vault: deny (denied: ")
