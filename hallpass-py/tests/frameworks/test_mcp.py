...
def test_real_vault(fw_mcp_vault: str) -> None:
    hp = vault_hallpass(fw_mcp_vault)
    results = []
    for user, path in ((ADMIN, "app/db"), (DANA, "app/db"), (ADMIN, "other/db")):
        server: MCPServer[Any] = MCPServer("fw-mcp-vault")
        ...
        guard(server, hp, {"write_secret": ("vault", "secret.write", "kv:secret/{path}")}, user=user)
        results += run(server, [("write_secret", {"path": path, "value": "x", "user_id": ADMIN})])
