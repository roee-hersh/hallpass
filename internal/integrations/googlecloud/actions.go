...
var (
	...
	objectRe    = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._/@:+=~-]*$`)
	...
	// fullNameRe is a full resource name as the troubleshooter takes it.
	fullNameRe = regexp.MustCompile(`^//[a-z][a-z0-9-]{0,62}\.googleapis\.com/[A-Za-z0-9_][A-Za-z0-9._@:/+=~-]*$`)
)
...
type ref struct {
	permission string
	resource   string
	// typ is the resource type as the caller wrote it.
	typ string
}
...
func parseRef(actionName string, r catalog.Resource) (ref, error) {
	...
	out := ref{resource: full, typ: r.Type}
	...
	if !contains(a.types, r.Type) && !hierarchyTypes[r.Type] {
...
func fullResourceName(r catalog.Resource) (string, error) {
	...
	case "project":
		if !projectRe.MatchString(id) {
			return "", invalid("project: id must be a project id (6-30 lowercase letters, digits and hyphens)")
		}
	...
	case "object":
		...
		if !ok || !bucketRe.MatchString(bucket) || !within(objectRe, object) {
			return "", invalid("object: id must be <bucket>/<object name>")
		}
		return "//storage.googleapis.com/projects/_/buckets/" + bucket + "/objects/" + object, nil
	case "dataset":
		parts := strings.Split(id, "/")
		if len(parts) != 2 || !projectRe.MatchString(parts[0]) || !within(datasetRe, parts[1]) {
			return "", invalid("dataset: id must be <project>/<dataset>")
		}
		return "//bigquery.googleapis.com/projects/" + parts[0] + "/datasets/" + parts[1], nil
	case "table":
		parts := strings.Split(id, "/")
		if len(parts) != 3 || !projectRe.MatchString(parts[0]) || !within(datasetRe, parts[1]) || !within(tableRe, parts[2]) {
	...
	case "secret":
		parts := strings.Split(id, "/")
		if len(parts) != 2 || !projectRe.MatchString(parts[0]) || !secretRe.MatchString(parts[1]) {
	...
	case "instance", "service", "cluster":
		parts := strings.Split(id, "/")
		if len(parts) != 3 || !projectRe.MatchString(parts[0]) || !locationRe.MatchString(parts[1]) || !shortNameRe.MatchString(parts[2]) {
	...
	case "name":
		if !within(fullNameRe, id) || strings.Contains(id, "/../") || strings.HasSuffix(id, "/..") {
